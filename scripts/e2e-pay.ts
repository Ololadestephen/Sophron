...
const product = process.env.E2E_PRODUCT ?? "risk-report";
const account = process.env.E2E_ACCOUNT ?? "0.0.1234";
const symbol = process.env.E2E_SYMBOL ?? "HBAR";
...
const query = product === "risk-report"
    ? `account=${encodeURIComponent(account)}`
    : `symbol=${encodeURIComponent(symbol)}`;
const url = `${serverUrl}/data/${product}?${query}`;
